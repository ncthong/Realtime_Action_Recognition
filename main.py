...
# Đọc và ghi các tập tin video (chỉ được thử nghiệm cho đầu vào webcam)
cap = choose_run_mode(args)
video_writer = set_video_writer(cap, write_fps=int(7.0))


# # tập tin txt lưu dữ liệu đã học
...
while cv.waitKey(1) < 0:
    ...
    if has_frame:
        ...
        # hiển thị giá trị FPS thời gian thực
        if (time.time() - start_time) > fps_interval:
            # Tính số lượng khung hình trong khoảng này. Nếu khoảng thời gian là 1 giây, đó là FPS.
            realtime_fps = fps_count / (time.time() - start_time)
            fps_count = 0  # xóa số fps
            start_time = time.time()
        fps_label = 'FPS:{0:.5f}'.format(realtime_fps)
